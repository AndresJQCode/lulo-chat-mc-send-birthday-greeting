...
export class TransationStatusCronjobService {
  ...
  async handleCron() {
    ...
    for (const transaction of transactions) {
      ...
      if (transactionWompi.length > 0 && transactionWompi[0].status === TransacionStatus.APPROVED) {
        await this.wcService.updateOrder(transactionWompi[0], transaction.paymentId);
        await this.transationsService.updateStatusTransaction(transaction.id, OrderStatus.COMPLETED);
      }
    }
